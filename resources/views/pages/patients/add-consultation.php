 ...
 */
assert(
  isset($patients) && is_array($patients),
  new Error('$patients must be an array')
);

assert(
  isset($consultationCauseCategories) && is_array($consultationCauseCategories),
  new Error('$consultationCauseCategories must be an array')
);

assert(
  isset($departments) && is_array($departments),
  new Error('$departments must be an array')
);

assert(
  isset($doctors) && is_array($doctors),
  new Error('$doctors must be an array')
);

$encodedPatients = json_encode(array_map(static fn(Patient $patient): array => [
  'value' => $patient->id,
...
<form
  ...
  <fieldset
    ...
    x-data='{
      selected_id: null,
      search: "",
      patients: JSON.parse(`<?= $encodedPatients ?>`),

      filteredPatients: []
    }'
    x-init="filteredPatients = patients"
    x-effect="filteredPatients = patients.filter(savedPatient => {
      return savedPatient.text.startsWith(search);
    })">
      ...
      <menu class="dropdown-menu dropdown-menu-end" style="width: 90%">
        ...
        <template x-for="patient in filteredPatients">
          <li
            ...
            :class="patient.value === selected_id && 'active'"
            @click="selected_id = patient.value">
            <button
              type="button"
              class="btn w-100 text-start"
              :class="patient.value === selected_id ? 'btn-primary' : 'btn-outline-primary'"
              x-text="patient.text">
            </button>
          </li>
    ...
    <!-- <div class="nice-select default_sel mb_30 w-100" tabindex="0">
      <span class="current">Select</span>
      <div class="nice-select-search-box">
        <input class="nice-select-search" placeholder="Search..." type="text">
      </div>
      <ul class="list">
        <li
          data-value="Nothing"
  ...
  <button
    id="register-btn"
    class="btn btn-primary d-none btn-lg mt-4 px-5 rounded-pill">
    Registrar
  </button>
</form>

<?php Flight::render(
  'forms/patient-register',
  ['action' => './pacientes?referido=/consultas/registrar']
) ?>

<script>
  const $consultationCauseSelect = document.querySelector(
    '[name="consultation_cause"]'
  )
  ...
  const $consultationTypeSelect = document.querySelector(
    '[name="consultation_type"]'
  )
  ...
  document
    .querySelector('[name="consultation_cause_category"]')
    .addEventListener('change', async event => {
      if (!causesByCategory[event.target.value]) {
        const input = `./api/causas-consulta/categorias/${event.target.value}`
      ...
      for (const cause of causesByCategory[event.target.value]) {
        html += `
          <option value="${cause.id}">
            ${cause.name.extended || cause.name.short}
          </option>
        `
      }
  ...
  $consultationCauseSelect.addEventListener('change', async event => {
    ...
    const input = `./api/pacientes/${$idCardSelect.value}/causas-consulta/${$consultationCauseSelect.value}`
    const response = await fetch(input)
